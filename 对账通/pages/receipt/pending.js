...
Page({
  ...
  map: function(e) {
    wx.navigateTo({
      url: '../receipt/map?bill_id=' + e.target.dataset.id,
    })
  },
  ...
  sub: function() {
    wx.request({
      url: app.data.baseUrl + 'checkPayMentInfo',
      header: {
        'content-type': 'application/x-www-form-urlencoded'
      },
      data: {
        ...
        bill_id: this.data.bill_id,
        payment_status: 2,
        bill_check_opcode: wx.getStorageSync('username'),
        bill_check_remark: this.data.ret_remark
      },
      success: (res) => {
